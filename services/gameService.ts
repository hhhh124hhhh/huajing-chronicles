import { AgeGroup, IntroData, QuizData, ModuleRecord, FinancialReport } from '../types';
...
export const generateIntroStory = async (
  ...
  try {
    ...
    try {
      ...
      const json = await aiService.generateJSON(promptContext, schema);
    ...
    try {
      imageUrl = await aiService.generateImage(imagePrompt);
    } catch (e) {
...
export const createAssistantChat = (ageGroup: AgeGroup, nickname: string, industry?: string): any => {
  ...
  return aiService.createChat(systemInstruction);
};
...
export const generateModuleQuiz = async (
  ...
  let systemInstruction = `
    ...
    【核心指令】
    请严格使用中文生成一个“命运抉择时刻”，包括情景、问题、3个选项和每个选项的结果。
    **根据玩家之前的剧情上下文调整难度和诱惑。**
    
    【关键要求】
    1. **完全中文输出**：所有内容必须使用中文，不得出现任何英文单词或短语。
    2. **选项与情景完全对应**：生成的3个选项必须与情景中描述的三个选择完全对应，详细体现每个选择的具体细节、人物关系和背景信息。
    3. **制造深度信息差**：每个选项都要有明确的短期利益和长期风险，不要让选项看起来一眼就能分出好坏。
    4. **突出道德困境**：选项要体现人性的挣扎，让玩家在道德和利益之间做出艰难抉择，详细描述每个选择可能带来的法律风险和道德后果。
    5. **选项描述要有深度**：每个选项的描述不能过于简单，要详细说明选择的背景、可能的后果和道德冲突，体现出每个选择的复杂性。
    6. **打乱顺序**：生成的 3 个选项必须打乱顺序。
    7. **叙事性结果**：每个选项的结果必须紧扣剧情，有明确的短期和长期影响，要体现出选择对玩家角色的人格、职业发展和人际关系的深远影响，详细描述选择后的具体发展和可能面临的后果。
    8. **符合游戏主题**：内容要与游戏的核心主题（人性、抉择、生存）紧密结合，突出华京市这个充满机遇与陷阱的现实主义都市的特点。
  ...
  if (ageGroup !== 'adult') {
      systemInstruction = `
        ...
        【核心指令】
        请严格使用中文为${ageGroup === 'child' ? '儿童' : '青少年'}玩家生成一个有趣的RPG故事选择，包括情景、问题、3个选项和每个选项的结果。
        **根据玩家之前的剧情上下文调整难度和诱惑。**
        ...
        2. **选项与情景完全对应**：生成的3个选项必须与情景中描述的三个选择完全对应，详细体现每个选择的具体细节、人物关系和背景信息。
        3. **制造深度信息差**：每个选项都要有明确的短期利益和长期风险，不要让选项看起来一眼就能分出好坏。
        4. **突出道德困境**：选项要体现人性的挣扎，让玩家在道德和利益之间做出艰难抉择，详细描述每个选择可能带来的后果和道德冲突。
        5. **选项描述要有深度**：每个选项的描述不能过于简单，要详细说明选择的背景、可能的后果和道德冲突，体现出每个选择的复杂性。
        6. **打乱顺序**：生成的 3 个选项必须打乱顺序。
        7. **叙事性结果**：每个选项的结果必须紧扣剧情，有明确的短期和长期影响，要体现出选择对玩家角色的人格、成长和人际关系的深远影响，详细描述选择后的具体发展和可能面临的后果。
        8. **符合游戏主题**：内容要与游戏的核心主题（人性、抉择、生存）紧密结合，突出华京市这个充满机遇与陷阱的现实主义都市的特点。
        9. **适合${ageGroup === 'child' ? '儿童' : '青少年'}**：语言要通俗易懂，符合${ageGroup === 'child' ? '儿童' : '青少年'}的认知水平，但仍要保持选项的深度和复杂性。
  ...
  try {
    ...
    return await aiService.generateJSON(systemInstruction + "\n生成 JSON 数据。", schema) as QuizData;
  } catch (error) {
...
export const updateNarrativeContext = async (
  ...
  const prompt = `
    根据最新的剧情发展更新故事摘要。
    当前故事："${currentContext}"
    新事件："${scenario}"。选择："${choice}"。结果："${outcome}"。
    任务：写一个1句话的摘要。使用黑色电影风格。
    请用中文生成。
  `;

  try {
    const newEntry = await aiService.generateText(prompt);
...
export const generateUserReport = async (history: Record<string, ModuleRecord>, userAge: number): Promise<FinancialReport> => {
  ...
  try {
    ...
    return await aiService.generateJSON(prompt, schema) as FinancialReport;
  } catch (e) {
